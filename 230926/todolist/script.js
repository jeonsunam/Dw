...
const addListItem = () => {
    ...
    // li에 사용자의 입력과 버튼 태그 추가
    li.innerHTML = `${input.value} <button class="list-delete">&#x2716</button>`;
    // li에 list-item 클래스 추가
    li.classList.add('list-item');
    // list에 li 요소 추가
    list.append(li);
    // 입력 후 검색버튼 누른 후에 검색창 비우기
    input.value = '';
    // 추가 버튼을 누른 후 사용자가 다시 검색창을 누르지 않아도 자동으로 입력할 수 있게끔 해준다
    input.focus();
    // localStorage에 저장
...
list.addEventListener('click', (e) => {
    ...
    // e.target.tagName과 비교할때는 대문자로 비교 / li요소를 클릭했을때
    ...
    if(e.target.tagName === 'BUTTON') {
        // parentElement : 요소의 부모를 가리킨다. / 삭제 버튼의 부모를 삭제(li요소 삭제)
